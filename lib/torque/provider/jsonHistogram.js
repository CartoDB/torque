...
var Profiler = require('../profiler');
  ...
  var jsonHistogram = function (options) {
    this._ready = false;
    this._tileQueue = [];
    this._filters = [];
  ...
  jsonHistogram.prototype = {
    ...
    getHeader:function(){
        var query_parts  = this.options.variables.map(function(variable){
          if(variable.type=='category'){
            return 'count(distinct('+variable.name+')) as '+variable.name+'_valueCount'
          }
          else{
            return 'min('+variable.name+') as '+variable.name+'_min , max('+variable.name+') as '+variable.name+'_max'
          }
        })
        var query = "select "+query_parts.join(",")+" from "+this.options.table;
        this.sql(query,this.processHeader.bind(this))
    },
    processHeader:function(result){

      if (result) {
        var details = JSON.parse(result.responseText).rows[0];
        var opts = {}
        this.options.variables.forEach(function(variable){
          console.log(variable)
        })
      } else {
        console.log("COUNT NOT GET HEADER")
      }
    },
    ...
    _filtersToSQL:function(filters){
      if(filters.length==0) return "";

      result = filters.map(function(filter){

        variable = this.options.variables.filter(function(v){return v.name= filter.name})[0]
        if(variable.type=='collection'){
          return variable.name+' in ['+filter.values+']'
        }
        else{
          return variable.name+' >  '+filter.min+' and '+variable.name+' < '+filter.max;

        }
      }.bind(this)).join(" and ")
      console.log("filter part is ", result)
      return " where "+result
    },

    histogramSQLForVariable: function(variable){
      if (variable.type=='category'){
        return [variable.name+"_hist as (",
          "select array_agg(b.bins) as bins, array_agg(b.vals) as vals from (select cte."+variable.name+" as bins, count(*) vals from cte group by "+variable.name+") as b)"
        ].join(" ")
      }
      else{
        return [variable.name+"_hist as(",
          " select array_agg(b.bins) as bins, array_agg(b.vals) as vals from(select width_bucket("+variable.name+",0,20,20) bins ,count(*) vals  from cte  group by 1 order by 1) as b)"
        ].join(" ")
      }
    },

    combineResultsSQL:function(variables){
      return "select "+variables.map(function(variable){return variable.name+"_hist.bins as "+variable.name+"_bins, "+variable.name+"_hist.vals as "+variable.name+"_vals"}).join(",")
             +" from "+variables.map(function(variable){return variable.name+"_hist"}).join(", ")
    },
     ...
     */
    proccessHistTile: function(rows,coord,zoom){
      return rows[0]
    },
    getTileHistograms:function(coord,zoom,callback){
      var start =[
       "WITH",
        "cte as(",
          "select * from "+this.options.user+"."+this.options.table,
          "WHERE "+this.options.user+"."+this.options.table+".the_geom_webmercator && CDB_XYZ_Extent("+coord.x+","+coord.y+", "+coord.zoom+")",
        "),"
      ].join(" ")

      var mid = this.options.variables.map(function(variable){
        return this.histogramSQLForVariable(variable);
      }.bind(this)).join(", ")

      var end  =  this.combineResultsSQL(this.options.variables);
      var query =  [start,mid,end].join(" ")
      this.sql(query, function (data) {
        if (data) {
          var rows = JSON.parse(data.responseText).rows;
          callback(this.proccessHistTile(rows, coord, zoom));
        } else {
          callback(null);
        }
      }.bind(this));
    ...
    getSQL: function() {
      var preFilters = this.options.sql || "select * from " + this.options.table;

      var query = preFilters + this._filtersToSQL(this._filters)
      console.log("query is  ARE ", query )
      return query
    },
