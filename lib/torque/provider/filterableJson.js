      ...
      for (var r = 0; r < rows.length; ++r) {
        ...
        for (var j = 0, len = steps.length; j < len; ++j) {
            ...
            if (this.options.fields[j].type === 'cat') {
              var m = this.categoryMapping[this.options.fields[j].name][v]
              if (!m) {
                this.categoryMapping[this.options.fields[j].name][v] = Object.keys(this.categoryMapping[this.options.fields[j].name]).length + 1; // do not use 0
              }
              v = m
            }
            rr.push([r, v]);
