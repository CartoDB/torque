...
var contour = require('./contour');
  ...
  torque.extend(PointRenderer.prototype, torque.Event, {
    ...
    _gridData: function(tile){
      ...
      this.globalGrid = Array.apply(null, Array(this.tileDimensions.h * valsPerTile)).map(Number.prototype.valueOf,0);
      for (var i = 0; i < this.globalGrid.length; i++){
        this.globalGrid[i] = Array.apply(null, Array(this.tileDimensions.w * valsPerTile)).map(Number.prototype.valueOf,0);
      }

      // to do invert
      for(var i =0; i < tile.renderData.length; i++){
        var x = tile.x[i], y = 256 - tile.y[i];
