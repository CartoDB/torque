...
torque.extend(IsoRenderer.prototype, torque.Event, {
  ...
  _gridData: function(tile){
  	if (!this.optimalZoom) this.optimalZoom = tile.z;
  	var zoomDif = tile.z - this.optimalZoom;
  	var currentResolution = 1 << (Math.sqrt(this.options.resolution) + zoomDif);
  	var valsPerTile = this.TILE_SIZE/currentResolution;
    ...
    for(var i = 0; i < tile.renderData.length; i++){
    	...
    	this.globalGrid[baseIndex.y + (256 - y) / currentResolution -1][baseIndex.x + x/currentResolution] = tile.renderData[i];
    }
  },
